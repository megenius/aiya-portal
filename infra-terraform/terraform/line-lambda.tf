...
resource "aws_lambda_function" "webhook_line_lambda" {
  filename      = "../lambda_function_line.zip"
  function_name = "webhook-line-chatbot-v2_${var.env}"
  # role          = aws_iam_role.webhook_lambda_role.arn
  role          = "arn:aws:iam::398684454127:role/lambda_full_access"
  handler       = "webhook-line-chatbot-v2_${var.env}.lambda_handler"
  runtime       = "python3.9"
  memory_size   = 512
  timeout       = 60
...
resource "aws_lambda_event_source_mapping" "line_sqs_to_lambda" {
  event_source_arn = aws_sqs_queue.webhook_line_sqs.arn
  function_name    = aws_lambda_function.webhook_line_lambda.arn
  batch_size       = 1
