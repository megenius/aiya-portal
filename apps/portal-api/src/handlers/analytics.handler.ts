import { readItems, readItem, updateItem, createItem, readUsers } from "@directus/sdk";
...
async function getVoucherIdsForPage(directus: any, pageId?: string | string[]): Promise<Set<string>> {
  ...
  try {
    const page = await directus.request(
      (readItem as any)("pages_liff", pid, {
        ...
        deep: { vouchers: { limit: -1 }, populars: { limit: -1 }, banner_vouchers: { limit: -1 } }
      } as any)
    );
    (page?.vouchers || []).forEach((v: any) => v?.vouchers_id?.id && result.add(String(v.vouchers_id.id)));
    (page?.populars || []).forEach((v: any) => v?.vouchers_id?.id && result.add(String(v.vouchers_id.id)));
    (page?.banner_vouchers || []).forEach((v: any) => v?.vouchers_id?.id && result.add(String(v.vouchers_id.id)));
  } catch (e) {
...
export const getAnalyticsTrends = factory.createHandlers(
  ...
  async (c) => {
    ...
    try {
      ...
      try {
        const profRes = await directus.request(readItems("profiles", {
      ...
      try {
        const claimsRes = await directus.request(readItems("vouchers_users", {
        ...
        if (pageVoucherIds.size > 0) {
          try {
            const codesRes = await directus.request(readItems("vouchers_codes", {
              fields: ["id", "voucher"],
            ...
            if (codes.length > 0) {
              const allowedCodeIds = new Set<string>(codes.map((vc: any) => String(vc.id)));
              ...
              const codeIdsAll = Array.from(new Set(
                claims.map((vu: any) => getId(vu.code)).filter((x): x is string => typeof x === 'string' && x.length > 0)
              ));
              const chunkSize = 500;
              ...
              for (let i = 0; i < codeIdsAll.length; i += chunkSize) {
                ...
                try {
                  const part = await directus.request(readItems("vouchers_codes", {
                    fields: ["id", "voucher"],
                    filter: { id: { _in: chunk } },
                    limit: -1
                  } as any));
            ...
            console.warn("profiles/trends: vouchers_codes filter failed, using global claims", codesErr);
      ...
      try {
        const eventsRes = await directus.request(readItems("user_events", {
          ...
          if (pid) {
            events = events.filter(ev => {
              ...
              if (typeof props === 'string') {
                try { props = JSON.parse(props); } catch { return false; }
              }
      ...
      const buildSeries = (rec: Record<string, number>) => {
        return Object.keys(rec).sort().map(k => ({ date: k, count: rec[k] }));
      };

      return c.json({
        range: { days: rangeDays, startDate: start.toISOString(), endDate: now.toISOString() },
    ...
    try {
      ...
      if (pid && pageVoucherIds.size > 0) {
        try {
          ...
          console.warn("leaderboards: vouchers_codes filter failed, using global claims", codesErr);
          // continue without page restriction
        }
      } else {
        // No page filter: minimize extra fetch by mapping from claims' code ids (smaller set)
        const codeIds = Array.from(new Set(
          claims.map((vu: any) => getId(vu.code)).filter((x): x is string => typeof x === 'string' && x.length > 0)
        ));
        if (codeIds.length > 0) {
          const codesRes = await directus.request(readItems("vouchers_codes", {
      ...
      let brandMeta: Record<string, { id: string, name: string, logo?: string }> = {};

      // 5) Top vouchers by clicks from user_events (event_type = 'voucher_click')
      const clickEventsRes = await directus.request(readItems("user_events", {
      ...
      clickEvents.forEach((ev: any) => {
        ...
        const voucherOk = !pid || pageVoucherIds.size === 0 || (vid && pageVoucherIds.has(vid));
      ...
      const metaVoucherIds = Array.from(new Set([...(voucherIds || []), ...clickVoucherIds]))
        .filter(id => !pid || pageVoucherIds.size === 0 || pageVoucherIds.has(id));
      if (metaVoucherIds.length > 0) {
        try {
          const vouchersRes = await directus.request(readItems("vouchers", {
            fields: [
              "id",
              "name",
              "cover",
          ...
          vouchers.forEach(v => {
            ...
            if (b?.id) {
              ...
              if (!brandMeta[bid]) brandMeta[bid] = { id: bid, name: b.name || 'Unknown', logo: b.logo };
            }
          });
        } catch (e) {
      ...
      const brandCountsClaims: Record<string, { id: string, name: string, claims: number }> = {};
      claims.forEach((vu: any) => {
        ...
        if (!brandCountsClaims[bid]) brandCountsClaims[bid] = { id: bid, name: bname, claims: 0 };
        brandCountsClaims[bid].claims += 1;
      });

      const brandCountsClicks: Record<string, { id: string, name: string, claims: number }> = {};
      Object.entries(voucherClicks).forEach(([vid, clicks]) => {
        const v = voucherById[vid];
        const b = v?.voucher_brand_id;
        const bid = b?.id ? String(b.id) : "unknown";
        const bname = b?.name || "Unknown";
        if (!brandCountsClicks[bid]) brandCountsClicks[bid] = { id: bid, name: bname, claims: 0 };
        brandCountsClicks[bid].claims += Number(clicks);
      });

      const brandCounts = Object.keys(brandCountsClaims).length > 0 ? brandCountsClaims : brandCountsClicks;
      ...
      if (clickVoucherIds.length > 0) {
        try {
          ...
          const vouchers = toArray(vouchersRes);
          vouchers.forEach(v => { voucherById[String((v as any).id)] = { ...(voucherById[String((v as any).id)]||{}), ...v }; });
        } catch {}
      }
      const topVouchersByClicks = Object.entries(voucherClicks)
        .map(([vid, clicks]) => ({ voucherId: vid, name: voucherById[vid]?.name || vid, clicks, cover: voucherById[vid]?.cover }))
      ...
      const categoryCounts: Record<string, { id: string, name: string, claims: number }> = {};
      const voucherIdsForCategories = metaVoucherIds;
      if (voucherIdsForCategories.length > 0) {
        try {
          const vvcRes = await directus.request(readItems("vouchers_voucher_categories", {
            fields: [
          ...
          vvc.forEach((row: any) => {
            ...
            voucherToCats[vid].push({ id: String(cat.id), name: cat.name || 'Unknown' });
          });
          const addCount = (vid: string, count: number) => {
            const cats = voucherToCats[vid] || [];
            if (cats.length === 0) {
              if (!categoryCounts['other']) categoryCounts['other'] = { id: 'other', name: 'Other', claims: 0 };
              categoryCounts['other'].claims += count;
            } else {
              cats.forEach(ca => {
                if (!categoryCounts[ca.id]) categoryCounts[ca.id] = { id: ca.id, name: ca.name, claims: 0 };
          ...
          if (Object.keys(categoryCounts).length === 0) {
            Object.entries(voucherClicks).forEach(([vid, clicks]) => addCount(vid, Number(clicks)));
          }
        } catch (e) {
          ...
          else total = Object.values(voucherClicks).reduce((s, n) => s + Number(n || 0), 0);
          if (total > 0) {
            categoryCounts['other'] = { id: 'other', name: 'Other', claims: total };
          }
        }
      }
      const categoryShareByClaims = Object.values(categoryCounts)
        .sort((a, b) => b.claims - a.claims);
      ...
      const uniqueUserIds = Array.from(new Set([...topUserIdsCollected, ...topUserIdsUsed]));
      let userMeta: Record<string, { id: string, name: string, avatar?: string }> = {};
      if (uniqueUserIds.length > 0) {
        try {
          const profilesRes = await directus.request(readItems("profiles", {
          ...
          profiles.forEach(p => {
            ...
            userMeta[String(p.id)] = { id: String(p.id), name, avatar: p.picture_url };
          });
        } catch (e) {
      ...
      return c.json({
        period: { days: rangeDays, startDate: start.toISOString(), endDate: now.toISOString() },
        topBrandsByClaims,
        topVouchersByClicks,
...
export const getAnalyticsOverview = factory.createHandlers(
  ...
  async (c) => {
    ...
    try {
      ...
      let totalUsers = 0;
      try {
        const profRes = await directus.request(readItems("profiles", { fields: ["id"], limit: -1 } as any));
      ...
      // 4) Claims today = vouchers_users with collected_date >= start of today
      ...
      try {
        const claimsRes = await directus.request(
          readItems("vouchers_users", {
            fields: ["id", "collected_date", "code"],
            filter: { collected_date: { _gte: startOfToday.toISOString() } as any },
            limit: -1
          })
        );
        let claims = toArray<any>(claimsRes);
        if (pid && pageVoucherIds.size > 0) {
          const codeIds = claims
            .map(vu => getId(vu.code))
          ...
          if (codeIds.length > 0) {
            ...
            const codes = toArray<any>(codesRes);
            codes.forEach(vc => { if (vc?.id && vc?.voucher) codeToVoucher[String(vc.id)] = String(vc.voucher); });
          }
          claims = claims.filter(vu => {
            const cid = getId(vu.code);
            const vid = cid ? codeToVoucher[String(cid)] : undefined;
            return !!vid && pageVoucherIds.has(vid);
          });
        }
        claimsToday = claims.length;
      } catch (e) {
        console.warn("vouchers_users today query failed", e);
        claimsToday = 0;
      }

      // 5) Events last hour = user_events with date_created >= one hour ago
      // If liff_page_id present, only count events that have event_properties.page_id = pid
      let eventsLastHour = 0;
      try {
        const eventsRes = await directus.request(
          readItems("user_events", {
            fields: ["id", "date_created", "event_properties"],
            filter: { date_created: { _gte: oneHourAgo.toISOString() } as any },
            limit: -1
          } as any)
        );
        let events = toArray<any>(eventsRes);
        ...
        console.warn("user_events last hour query failed", e);
        eventsLastHour = 0;
      }
