...
const ChannelEditor: React.FC<ChannelEditorProps> = ({
  ...
  const [input, setInput] = useState<Channel>(channel);
    ...
    <>
      ...
      >
        <div className="flex justify-between items-center py-3 px-4 border-b">
          <h3 id="example-label" className="font-bold text-gray-800">
            {channel?.name}<span className='text-gray-400 ps-2'>{id || channel?.id}</span>
          </h3>
          <button
            type="button"
            className="size-8 inline-flex justify-center items-center gap-x-2 rounded-full border border-transparent bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-none focus:bg-gray-200 disabled:opacity-50 disabled:pointer-events-none"
            aria-label="Close"
            data-hs-overlay={`#${id || channel?.id}`}
          >
