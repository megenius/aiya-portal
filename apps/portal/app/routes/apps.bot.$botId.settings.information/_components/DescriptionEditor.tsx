...
const DescriptionEditor: React.FC<DescriptionEditorProps> = ({ bot }) => {
  const updateBot = useBotUpdate()
  const [input, setInput] = React.useState(bot?.product_description as string)
    ...
    updateBot.mutateAsync(
      {
        variables: {
          ...
          data: { product_description: input }
        }
      })
      .then(res => {
              ...
              {input !== bot.product_description && (
                <div className="flex gap-x-3">
                  ...
                  <button
                    ...
                    onClick={() => setInput(bot?.product_description as string)}
                  >
                    Cancel
                  </button>
                </div>
