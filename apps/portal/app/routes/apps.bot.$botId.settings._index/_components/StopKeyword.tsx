...
interface StopKeywordProps {
  bot: Bot
}

const StopKeyword: React.FC<StopKeywordProps> = ({ bot }) => {
  const updateBot = useBotUpdate()
  const [input, setInput] = React.useState(bot?.stop_keyword as string)
              ...
              {input !== bot.stop_keyword && (
                <div className="flex gap-x-3">
                  ...
                  <button
                    ...
                    onClick={() => setInput(bot?.stop_keyword as string)}
                  >
                    Cancel
                  </button>
                </div>
