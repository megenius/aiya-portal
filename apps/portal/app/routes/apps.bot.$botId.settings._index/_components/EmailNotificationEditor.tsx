...
const EmailNotificationEditor: React.FC<EmailNotificationEditorProps> = ({ bot }) => {
  const updateBot = useBotUpdate()
  const [input, setInput] = React.useState(bot?.name as string)
              ...
              {input !== bot.name && (
                <div className="flex gap-x-3">
                  ...
                  <button
                    ...
                    onClick={() => setInput(bot?.name as string)}
                  >
                    Cancel
                  </button>
                </div>
