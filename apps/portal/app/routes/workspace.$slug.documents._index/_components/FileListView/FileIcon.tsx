...
export const FileIcon: React.FC<FileIconProps> = ({ fileType, className = 'shrink-0 size-7 mx-auto' }) => {
  // Convert file type to lowercase for case-insensitive matching
  ...
  const getIconType = (type: string): string => {
    if (type.includes('pdf')) {
      return 'pdf';
    } else if (['jpg', 'jpeg', 'png', 'gif', 'svg', 'webp'].some(ext => type.includes(ext))) {
      return 'image';
    } else if (['doc', 'docx', 'txt', 'rtf'].some(ext => type.includes(ext))) {
      return 'document';
    } else if (['mp4', 'mov', 'avi', 'wmv'].some(ext => type.includes(ext))) {
      return 'video';
    } else if (type === 'folder') {
      return 'folder';
    } else if (['xls', 'xlsx'].some(ext => type.includes(ext))) {
      return 'xls';
  ...
  switch (iconType) {
    case 'pdf':
      return (
        <svg {...iconProps} viewBox="0 0 400 492">
    ...
    case 'image':
      return (
        <svg {...iconProps} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-image-icon lucide-image"><rect width="18" height="18" x="3" y="3" rx="2" ry="2" /><circle cx="9" cy="9" r="2" /><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21" /></svg>
      );
