...
const IntentImporter: React.FC<IntentImporterProps> = ({ existingIntents, onImport }) => {
  ...
  const handleImport = () => {
    ...
    transformedData.forEach((item, index) => {
      if (!item.name || item.questions.length === 0 || item.responses.length === 0) {
        errors.push(`Row ${index + 1}: Missing required fields or all responses/questions are empty`);
        return;
      }
        ...
        <div className="flex items-center justify-center w-full p-12  bg-white border border-dashed border-gray-300 rounded-xl">
          ...
          <div className="text-center">
            ...
            <div className="mt-4 flex flex-wrap justify-center text-sm leading-6 text-gray-600">
              ...
              <span
                className="relative cursor-pointer bg-white font-semibold text-blue-600 hover:text-blue-700 rounded-lg decoration-2 hover:underline focus-within:outline-none focus-within:ring-2 focus-within:ring-blue-600 focus-within:ring-offset-2"
              >
                <span>browse</span>
                <input
                  id="dropzone-file"
