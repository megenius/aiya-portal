    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 shadow-xs rounded-xl p-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <p className="text-sm font-semibold text-gray-900">{stats.active} / {stats.total}</p>
        </div>
        {(() => {
          const total = Math.max(1, stats.total || 0);
          const pct = Math.min(100, Math.max(0, Math.round((stats.active / total) * 100)));
          return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {(() => {
          ...
          const donuts = [
            { label: 'Collection Rate', value: clamp(stats.collectionRate), color: '#2563eb', bg: '#dbeafe' },
            { label: 'Redemption Rate', value: clamp(stats.redemptionRate), color: '#7c3aed', bg: '#ede9fe' },
            { label: 'View → Collection', value: clamp(stats.viewToCollectionRate), color: '#f59e0b', bg: '#fffbeb' }
          ];
          ...
          return donuts.map((d, idx) => (
            <div key={idx} className="bg-white border border-gray-200 shadow-xs rounded-xl p-6 flex items-center gap-4">
              <svg width="96" height="96" viewBox="0 0 96 96" className="shrink-0">
                <circle cx={cx} cy={cy} r={radius} stroke={d.bg} strokeWidth={stroke} fill="none" />
                <circle
                  cx={cx}
                  cy={cy}
                  r={radius}
                ...
                />
                <text x="50%" y="50%" dominantBaseline="middle" textAnchor="middle" className="fill-gray-900 text-xl font-bold">
                  {d.value}%
                </text>
              </svg>
              <div>
      ...
      {/* Today vs This Week */}
      <div className="bg-white border border-gray-200 shadow-xs rounded-xl p-6">
        <p className="text-sm font-medium text-gray-600 mb-3">Activity Summary</p>
        {(() => {
          ...
          return (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full bg-blue-500" style={{ width: `${toPct}%` }} />
                </div>
              </div>
              <div>
                <div className="flex items-center justify-between mb-1">
                ...
                <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full bg-indigo-500" style={{ width: `${wkPct}%` }} />
                </div>
              </div>
            </div>
          );
