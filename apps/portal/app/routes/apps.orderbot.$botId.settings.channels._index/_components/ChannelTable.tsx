...
import { Bot, BotChannelStatus, Channel, Orderbot } from '~/@types/app';
import { useOrderbotUpdate } from '~/hooks/orderbot/useOrderbotUpdate';
import { useOrderbotChannelDelete } from '~/hooks/orderbot/useOrderbotChannelDelete';
import { useOrderbotChannelInsert } from '~/hooks/orderbot/useOrderbotChannelInsert';
...
const ChannelTable: React.FC<ChannelTableProps> = ({ bot, channels }) => {
  const insertBotChannel = useOrderbotChannelInsert();
  ...
  const handleConnectChannel = async (channel: Channel) => {
    insertBotChannel.mutateAsync({ variables: { orderbot_id: bot.id as string, channel_id: channel.id as string } })
      .then(async () => {
