...
const FullScreenBalls = () => {
  ...
  // Initialize 30 balls with random positions and velocities
  ...
  // Update balls when dimensions change
  useEffect(() => {
  ...
  const selectWinner = () => {
    ...
    const flashInterval = setInterval(() => {
      ...
      if (counter < totalFlashes) {
        ...
        const finalWinnerIndex = Math.floor(Math.random() * 30);
        const winnerId = finalWinnerIndex + 1;
    ...
    <div className="w-full h-screen flex flex-col overflow-hidden">
      ...
      >
        ...
        {showWinnerDisplay && winnerBall && (
          <div className="absolute top-0 left-0 right-0 mt-16 flex justify-center z-45">
        ...
        {balls.map(ball => (
          <div
            key={ball.id}
            className={`absolute rounded-full ${
              ball.isWinner ? 'shadow-lg' : ''
            } ${showWinnerDisplay && !ball.isWinner ? 'opacity-0' : ball.colorData.class} 
            ...
            style={{
              ...
              fontSize: ball.isWinner && showWinnerDisplay ? '3rem' : '1.5rem',
