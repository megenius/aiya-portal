...
export const WorkspaceTable: React.FC<WorkspaceTableProps> = ({ workspaces, onRowClick }) => {
  return (
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-neutral-700">
            {workspaces?.map((workspace, index) => (
              <tr key={index} className='cursor-pointer' onClick={() => onRowClick && onRowClick(workspace)}>
                <td className="size-px whitespace-nowrap pe-4 py-3">
                  <div className="w-full flex items-center gap-x-3">
                    <Avatar src={getDirectusFileUrl(workspace.avatar as string)} firstName={workspace.name as string} />
                    <div className="grow">
