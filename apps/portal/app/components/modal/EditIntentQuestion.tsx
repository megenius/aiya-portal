...
interface EditIntentQuestionModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onQuestionEdited: (updatedQuestions: string[]) => void;
...
const EditIntentQuestionModal: React.FC<EditIntentQuestionModalProps> = ({
  ...
  const handleSave = () => {
    ...
    if (isNewQuestion) {
      ...
      updatedQuestions[questionIndex] = input.trim();
    }
            ...
            <div className="p-4 flex justify-between items-center">
              {!isNewQuestion && (
                <button
                  ...
                  className="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-red-500 bg-white text-red-500 shadow-sm hover:bg-red-50 disabled:opacity-50 disabled:pointer-events-none"
                >
                  Remove
                </button>
              )}
              <div className={`flex gap-x-2 ${isNewQuestion ? 'ml-auto' : ''}`}>
                ...
                <button
                  type="submit"
                  disabled={isNewQuestion ? !input.trim() : !isDirty}
